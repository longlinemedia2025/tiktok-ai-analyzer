...
def analyze_video():
    ...
    # === SAFETY: fallback for cloud ===
    if not os.path.exists(video_path):
        mock_meta = {
            "duration_seconds": 20,
            "resolution": (1080, 1920),
            "aspect_ratio": 9/16,
            "mean_brightness_first_frame": 100
        }
        heuristics = heuristic_score(mock_meta)
        ai_output = generate_ai_insights(mock_meta, keywords, tone, niche)
        return jsonify({
            "status": "warning",
            "message": f"Video not found ({video_path}). Simulated data used.",
            "results": {
    ...
    # Step 5: Return full JSON (same format as before)
    return jsonify({
        "status": "success",
        "results": results,
